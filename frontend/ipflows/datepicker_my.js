 ...
 * Edited by Michal Potfaj (changed some date picker preferences & added seconds).
 */
...
with (Control.DatePicker) {
	...
	Locale['en_my'] = i18n.createLocale('my', 'en');
...
//======================COPY
                        ...
                        cell.style.textAlign = 'right';
                        cell.colSpan = 5;
                        cell.innerHTML = '<i>'+this.tr('Exact seconds')+':</i>';

                        cell = row.insertCell(1);
                        cell.className = 'othersecond';
                        var otherInput2 = document.createElement('input');
                        otherInput2.type = 'text';
                        otherInput2.maxLength = 2;
                        otherInput2.style.width = '2em';
                        var inputTimeout2 = null;
                        otherInput2.onkeyup = function(e) {
                                                if (!isNaN(otherInput2.value)) {
                                                        inputTimeout2 = setTimeout(function() {
                                                                        this.currentDate.setSeconds(otherInput2.value);
                                                                        this.dateChanged(this.currentDate);
                                                                }.bind(this), 500);
                                                }
                                        }.bindAsEventListener(this);
                        otherInput2.onkeydown = function(e) {
                                                if (e.keyCode == Event.KEY_RETURN)
                                                        if (this.options.onSelect) this.options.onSelect(this.currentDate);
                                                if (inputTimeout2)
                                                        clearTimeout(inputTimeout2)
                                        }.bindAsEventListener(this);
                        otherInput2.onclick = otherInput2.select;
                        otherInput2.onfocus = this.releaseKeys.bindAsEventListener(this);
                        otherInput2.onblur = this.captureKeys.bindAsEventListener(this);
                        this.otherSeconds = otherInput2;
                        cell.appendChild(otherInput2);
                        if (!this.options.use24hrs)
                                cell = row.insertCell(2);
	...
	dateClickedListener: function(date, timeOverride) {
		...
		return function(e) {
				if (!timeOverride) {
					...
					dateCopy.setSeconds(this.currentDate.getSeconds());
				}
...
//======COPY
        secondClickedListener: function(seconds) {
                return function(e) {
